...
class InventoryTransfer(models.Model):
    ...
    def is_transfer_possible(self):
        ...
        inventory_item = InventoryItem.objects.filter(product=self.product, location=self.from_location).first()
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.pk:
            ...
            if self.status == 'completed':
                # Deduct from source
                source_inventory = InventoryItem.objects.filter(product=self.product, location=self.from_location).first()
            ...
            if previous.status != self.status:
                if self.status == 'completed' and previous.status != 'completed':
                    ...
                    if self.is_transfer_possible():
                        ...
                        raise ValidationError("Not enough stock in source warehouse to complete transfer.")
                elif self.status == 'failed' and previous.status == 'completed':
                    # Reverse the transfer
                    source_inventory = InventoryItem.objects.filter(product=self.product, location=self.from_location).first()
                    dest_inventory = InventoryItem.objects.filter(product=self.product, location=self.to_location).first()
                    if dest_inventory and dest_inventory.quantity >= self.quantity:
                        ...
                        raise ValidationError("Cannot reverse transfer due to insufficient stock in destination warehouse.")
            super().save(*args, **kwargs)
