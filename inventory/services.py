...
class InventoryService:
    ...
    def add_stock(product, warehouse, quantity):
        item, created = InventoryItem.objects.get_or_create(product=product, location=warehouse)
...
class TransferService:
    ...
    def transfer_stock(product, from_warehouse, to_warehouse, quantity, user):
        if InventoryItem.objects.filter(product=product, location=from_warehouse, quantity__gte=quantity).exists():
            inventory_item = InventoryItem.objects.get(product=product, location=from_warehouse)
            StockAdjustmentService.adjust_stock(inventory_item, 'remove', quantity, 'Transfer to another warehouse', user)
            
            to_inventory_item, _ = InventoryItem.objects.get_or_create(product=product, location=to_warehouse)
            StockAdjustmentService.adjust_stock(to_inventory_item, 'add', quantity, 'Received from transfer', user)
