...
class LogisticsReports:
    def delayed_shipments_report(self, start_date, end_date):
        return Shipment.objects.delayed_shipments().filter(created_at__range=(start_date, end_date))
    ...
    def get_recent_deliveries():
        return Shipment.objects.filter(status='delivered').order_by('-delivered_date')[:10]
...
class ReturnReports:
    ...
    def get_recent_returns():
        return ReturnShipment.objects.filter(status='received').order_by('-received_at')[:10]
    

class ShipmentReports:
    def average_delivery_time(self):
        delivered_shipments = Shipment.objects.filter(status='delivered')
        return delivered_shipments.aggregate(average_time=models.Avg(models.F('estimated_arrival') - models.F('shipped_date')))