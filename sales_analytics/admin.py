...
class SalesReportAdmin(admin.ModelAdmin):
    list_display = ['report_date', 'total_sales', 'total_orders', 'total_customers', 'average_order_value', 'highest_selling_product', 'total_products_sold']
    list_filter = ['report_date', 'total_sales']
    search_fields = ['report_date']
    inlines = [SalesByProductInline, SalesByCustomerSegmentInline]
    readonly_fields = ['total_sales', 'total_orders', 'total_customers', 'average_order_value']
    ...
    def total_products_sold(self, obj):
        return obj.sales_by_product.aggregate(total=models.Sum('total_units_sold'))['total'] or 0
