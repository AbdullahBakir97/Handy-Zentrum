...
class RepairOrderForm(forms.ModelForm):
    class Meta:
        model = RepairOrder
        fields = [
            'shop', 'device_type', 'device_name', 'issue', 'total_price', 'expenses', 
            'status', 'details', 'customer_name', 'customer_contact', 'payment_received', 
            'payment_pending_reason', 'completion_time'
        ]
        widgets = {
...
class OrderForm(forms.ModelForm):
    class Meta:
        model = Order
        fields = ['customer', 'status', 'shipping_address', 'total_amount', 'payment_status']
        widgets = {
            'status': forms.Select(attrs={'class': 'form-control'}),
