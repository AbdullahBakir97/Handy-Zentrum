...
from inventory.models import Product, Category, Warehouse, InventoryItem, StockAdjustment, InventoryTransfer
from logistics.models import Shipment, LogisticsInteraction, ReturnShipment
from orders.models import Order, OrderItem, Payment
from customers.models import Customer, CustomerInteraction, LoyaltyProgram
from sales_analytics.models import SalesReport, SalesByProduct, SalesByCustomerSegment
...
def create_orders(customers, products, n=50):
    ...
    for _ in range(n):
        order = Order.objects.create(
            customer=random.choice(customers),
            status=random.choice(['pending', 'processed', 'shipped', 'delivered', 'canceled']),
    ...
    for shipment in shipments:
        if shipment.status == 'returned':
            ReturnShipment.objects.create(
                ...
                status=random.choice(['initiated', 'in_transit', 'received', 'refunded'])
            )
