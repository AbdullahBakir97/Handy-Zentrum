...
class ProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Product
        fields = ['id', 'name', 'subtitle', 'description', 'category', 'brand', 'base_price', 'flag', 'slug', 'sku', 'tags', 'image', 'is_active', 'created_at', 'updated_at']
...
class ProductVariantSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'product', 'color', 'size', 'sku', 'price', 'weight', 'dimensions', 'stock', 'variant_group', 'is_active']

    def validate(self, data):
        # Ensure the uniqueness of the color and size combination per product
        if ProductVariant.objects.filter(product=data.get('product'), color=data.get('color'), size=data.get('size')).exists():
            raise serializers.ValidationError("This combination of color and size already exists for the selected product.")
        return data
