...
class ProductForm(forms.ModelForm):
    class Meta:
        model = Product
        fields = ['name', 'subtitle', 'description', 'category', 'brand', 'base_price', 'flag', 'slug', 'sku', 'tags', 'image']
...
class ProductVariantForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['product', 'color', 'size', 'sku', 'price', 'weight', 'dimensions', 'stock', 'variant_group']
    ...
    def clean(self):
        ...
        if ProductVariant.objects.filter(product=product, color=color, size=size).exists():
            raise forms.ValidationError("This combination of color and size already exists for the selected product.")
        return cleaned_data
