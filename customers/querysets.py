...
class CustomerQuerySet(models.QuerySet):
    ...
    def with_interactions(self):
        return self.annotate(interactions_count=Count('customer_interactions')).filter(interactions_count__gt=0)

    def without_interactions(self):
        return self.annotate(interactions_count=Count('customer_interactions')).filter(interactions_count=0)
