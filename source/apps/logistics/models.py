...
class Shipment(models.Model):
    product = models.ForeignKey(
        Product, on_delete=models.CASCADE, related_name="shipments"
    )
    quantity = models.IntegerField()
    origin = models.ForeignKey(
        Warehouse, related_name="outgoing_shipments", on_delete=models.CASCADE
    )
    ...
    status = models.CharField(
        ...
        choices=[
            ...
            for status in LOGISTICS_SETTINGS["SHIPMENT_STATUSES"]
        ],
        default="pending",
    )
...
class LogisticsInteraction(models.Model):
    shipment = models.ForeignKey(
        Shipment, on_delete=models.CASCADE, related_name="logistics_interactions"
    )
    ...
    def log_interaction(self, interaction_type, notes=None):
        ...
        LogisticsInteraction.objects.create(
            shipment=self.shipment, interaction_type=interaction_type, notes=notes
        )


class ReturnShipment(models.Model):
    shipment = models.OneToOneField(
        Shipment, on_delete=models.CASCADE, related_name="return_details"
    )
    ...
    def clean(self):
        ...
        if self.status == "received" and not self.received_at:
            raise ValidationError(
                "Received date must be set when the return status is 'received'."
            )
        if self.status != "received" and self.received_at:
            raise ValidationError(
                "Received date can only be set when the return is marked as 'received'."
            )
