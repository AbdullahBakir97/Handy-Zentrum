...
class Category(models.Model):
    ...
    def __str__(self):
        return (
            f"{self.name}"
            if not self.parent
            else f"{self.name} (Subcategory of {self.parent.name})"
        )
...
class Product(models.Model):
    ...
    category = models.ForeignKey(
        Category, on_delete=models.CASCADE, related_name="products_in_category"
    )
    ...
    def total_stock(self):
        ...
        return (
            self.inventory_records.aggregate(total=models.Sum("quantity"))["total"] or 0
        )
    ...
    def update_variant_skus(self):
        ...
        for variant in self.variants.all():
            expected_variant_sku = (
                f"{self.sku}-{variant.color[:1].upper()}-{variant.size[:1].upper()}"
            )
...
class ProductVariant(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        expected_sku = (
            f"{self.product.sku}-{self.color[:1].upper()}-{self.size[:1].upper()}"
        )
