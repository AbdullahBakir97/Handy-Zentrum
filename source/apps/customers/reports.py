...
def interaction_report():
    return (
        Customer.objects.annotate(interaction_count=Count("customer_interactions"))
        .values("first_name", "last_name", "interaction_count")
...
def customer_retention_report():
    ...
    active_customers = Customer.objects.filter(
        customer_interactions__interaction_date__gte=one_year_ago
    ).distinct()
    total_customers = Customer.objects.count()
    ...
    return {
        ...
        "retention_rate": (
            (active_customers.count() / total_customers) * 100 if total_customers else 0
        ),
    }


def inactive_customers_report():
    return Customer.objects.filter(customer_interactions__isnull=True).values(
