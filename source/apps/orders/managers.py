...
class RepairOrderManager(models.Manager):
    ...
    def unpaid_customer_orders(self):
        ...
        return self.filter(
            payment_received=False, payment_pending_reason="Awaiting Customer Pickup"
        )

    def unpaid_shop_orders(self):
        """Orders awaiting payment from another shop."""
        return self.filter(
            payment_received=False, payment_pending_reason="Sent to Other Shop"
        )

    def completed_orders(self):
        """Orders marked as completed (paid or unpaid)."""
        return self.filter(
            status__in=["completed", "customer_pickup", "sent_to_other_shop"]
        )
